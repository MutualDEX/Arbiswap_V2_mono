...
export const serializeParams = async (
  ...
  addressToIndex: (address: string) => Promise<number> = ()=>(new Promise((exec)=>exec(-1)))
) => {
    ...
    if (Array.isArray(param)) {
      ...
      if (isAddress(paramArray[0])) {
        ...
        const indices = await Promise.all(
          paramArray.map(async address => (await addressToIndex(address as string)))
        )        
        if (indices.every(i => i > -1 )) {
          paramArray = indices as number[]
          formattedParams.push(new Uint8Array([1]))
          paramArray.forEach((value) => {
            formattedParams.push(hexZeroPad(BigNumber.from(value).toHexString(), 4))
          })
        } else {
      ...
      if (isAddress(param)) {
        ...
        if ( index > -1) {
          formattedParams.push(new Uint8Array([1]))
          formattedParams.push(hexZeroPad(BigNumber.from(index).toHexString(), 4))
